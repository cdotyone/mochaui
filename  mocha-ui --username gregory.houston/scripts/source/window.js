...
MochaUI.Window = new Class({
	...
	*/	
	newWindow: function(properties){ // options is not doing anything
		...
		//alert(this.$events.onContentLoaded);
		
		//alert(MochaUI.serialize(this.$events));

		//	var mytest = this.$events;
		//	mytest = mytest.toString();
		//	alert(mytest);
		//	alert(this.$events);
		//	mytest.each(function(myevent){
		//		alert(myevent);
		//	});
			
			MochaUI.Windows.instances.set(this.options.id, this);
			//MochaUI.Windows.instances.set(this.options.id, {'options': this.events});
		}
	...
	*/	
	drawWindow: function(windowEl, shadows) {
		...
		if ( shadows != false ) {
			...
			MochaUI.roundedRect(ctx, 2, 2, width - 4, height - 4, this.options.cornerRadius, [0, 0, 0], 0.28);
		}
