...
MUI.Stepper = new NamedClass('MUI.Stepper', {
	...
	draw: function(container){
		...
		if(lbl) this.el.label = lbl.set('text', tle).set('for', options.id);
		...
		if(options.formData) value = MUI.getData(options.formData, (options.valueField ? options.valueField : options.id)); // get value from data if bound to a dataset
		if(value === null) value = this.el.input.get('value'); // pull value from input field if we don't have one yet
		...
		var addToContainer = function(){
			...
			if (this.el.element.getParent() === null){
				if (options.clearContainer) container.empty();
    ...
    attachEvents: function(){
        ...
        if(!!this.eventsAttached || !this.el.input) return this;
        ...
        this.el.up.addEvent('click', function(e){
            e.stop();
            if(iterator.hasNext()) this.setValue(iterator.next());
        }.bind(this));
        
        this.el.down.addEvent('click', function(e){
            e.stop();
            if(iterator.hasPrevious()) this.setValue(iterator.previous());
        }.bind(this));
    ...
    drawUpdate: function(){
        ...
        if(iterator.hasNext()) this.el.up.removeClass('disabled');
        else  this.el.up.addClass('disabled');
        
        if(iterator.hasPrevious()) this.el.down.removeClass('disabled');
        else  this.el.down.addClass('disabled');
	...
	getFormTitle: function(){
		var options = this.options; // local var options for better js compression
		if (options.formTitle) return options.formTitle;
		if (options.formTitleField) return MUI.getData(options.formData, options.formTitleField);
		if (options.formData) return MUI.getData(options.formData, options.id);
		return options.id;
	},
    ...
    validateValue: function(value){
        ...
        if(iterator.validate(value)) return true;
