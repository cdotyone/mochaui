...
var initializeWindows = function(){
	...
	MUI.checkBoxGrid = function(container){
		MUI.create({
	...
	MUI.selectListBuilder = function(container){
		MUI.create({
	...
	MUI.imageButtonBuilder = function(container){
	...
	MUI.textAreaBuilder = function(container){
		MUI.get(container).el.content.empty();
	...
	MUI.textBoxBuilder = function(container){
	...
	// Examples > Tests
	MUI.serverResponseWindow = function(response){
		new MUI.Window({
			id: 'serverResponse',
			content: response,
			width: 350,
					...
					onLoaded: function(){
						...
						if ($('wcbgLink')){
							$('wcbgLink').addEvent('click', function(e){
								...
								MUI.checkBoxGrid('cbgWindow');
							});
						}
						if ($('pslLink')){
							$('pslLink').addEvent('click', function(e){
								e.stop();
								MUI.selectListBuilder('mainPanel');
							});
						}
						if ($('wslLink')){
							$('wslLink').addEvent('click', function(e){
								...
								MUI.selectListBuilder('slWindow');
							});
						}
						if ($('pibLink')){
							$('pibLink').addEvent('click', function(e){
								e.stop();
								MUI.imageButtonBuilder('mainPanel');
							});
						}
						if ($('wibLink')){
							$('wibLink').addEvent('click', function(e){
								...
								MUI.imageButtonBuilder('wiWindow');
							});
						}
